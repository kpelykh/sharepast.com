package com.sharepast.restlet.services;

import com.sharepast.constants.LogonConstants;
import com.sharepast.restlet.exceptions.ClientResourceException;
...
import com.sharepast.restlet.freemarker.JsonRepresentationFactory;
import com.sharepast.restlet.freemarker.RepresentationFactory;
    ...
    public Representation getRepresentation(Status status, Request request, Response response) {
        ...
        if (status.equals(Status.CLIENT_ERROR_UNAUTHORIZED)) {
            ...
            if (request.getAttributes().get(LogonConstants.LOGON_TARGET_URI_NAME) == null) {
                //in case if user just clicked on login button, redirect to /home page
                if (request.getResourceRef().getPath().startsWith(LogonUtils.getLoginUri()))
                    dataModel.put(LogonConstants.LOGON_TARGET_URI_NAME, LogonUtils.getHomeUri());
                else
            ...
            if (request.getAttributes().get(LogonConstants.LOGON_TARGET_QUERY_NAME) == null &&
                    !request.getResourceRef().getPath().startsWith(LogonUtils.getLoginUri())) {
