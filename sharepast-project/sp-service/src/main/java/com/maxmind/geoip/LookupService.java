...
public class LookupService {
  ...
  public void init() throws IOException
  {
    ...
    if( !databaseFile.exists() )
    {
      ...
      LOG.info( String.format( "found geoip database at %s", databaseFilePath ) );
  ...
  public GeoIPLocation getLocation(InetAddress addr) {
    return getLocation(bytesToLong(addr.getAddress()));
  }

  // for GeoIP City only

  public GeoIPLocation getLocation(String str) {
    if (dnsService == 0) {
      InetAddress addr;
      try {
  ...
  public GeoIPLocation getLocationwithdnsservice(String str) {
    GeoIPLocation record = new GeoIPLocation();
    String key;
    String value;
    ...
    while (st.hasMoreTokens()) {
      ...
      if (key.equals("dm") || key.equals("me")) {
        try {
          ...
          record.setDmaCode(record.getMetroCode());
        } catch (NumberFormatException e) {
  ...
  public synchronized GeoIPLocation getLocation(long ipnum) {
    int record_pointer;
    ...
    try {
      ...
      // get country
      record.setCountryCode(countryCode[unsignedByteToInt(record_buf[0])]);
      record.setCountryName(countryName[unsignedByteToInt(record_buf[0])]);
      record_buf_offset++;
      ...
      while (record_buf[record_buf_offset + str_length] != '\0')
        str_length++;
      if (str_length > 0) {
        record.setRegion(new String(record_buf, record_buf_offset, str_length));
      }
      ...
      while (record_buf[record_buf_offset + str_length] != '\0')
        str_length++;
      if (str_length > 0) {
        record.setCity (new String(record_buf, record_buf_offset, str_length, "ISO-8859-1"));
      }
      record_buf_offset += str_length + 1;
      str_length = 0;
      ...
      while (record_buf[record_buf_offset + str_length] != '\0')
        str_length++;
      if (str_length > 0) {
        record.setPostalCode(new String(record_buf, record_buf_offset, str_length));
      }
      record_buf_offset += str_length + 1;
      ...
      if (databaseType == DatabaseInfo.CITY_EDITION_REV1) {
        ...
        if (record.getCountryCode().equals("US")) {
          record_buf_offset += 3;
          for (j = 0; j < 3; j++)
  ...
  public void setDbOptions(int dbOptions) {
